...
public class LibroResource {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<LibroDTO> getLibro(@PathVariable(name = "id") final Integer id) {
        return ResponseEntity.ok(libroService.get(id));
    }

    @PostMapping
    @ApiResponse(responseCode = "201")
    public ResponseEntity<Integer> createLibro(@RequestBody @Valid final LibroDTO libroDTO) {
        final Integer createdId = libroService.create(libroDTO);
        return new ResponseEntity<>(createdId, HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Integer> updateLibro(@PathVariable(name = "id") final Integer id,
    ...
    public ResponseEntity<Void> deleteLibro(@PathVariable(name = "id") final Integer id) {
